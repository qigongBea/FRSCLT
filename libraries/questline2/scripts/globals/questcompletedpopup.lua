...
local QuestCreatedPopup, super = Class("Object", "QuestCompletedPopup")

---@param quest Quest
function QuestCreatedPopup:init(quest)
    super.init(self, -SCREEN_WIDTH/2, 0, 280, SCREEN_HEIGHT)
    ...
    self.box:addChild(Text("[font:main,16][color:yellow]You Completed a Quest!"))
    local box_size_w = self.box.x_sizing:getWidth() - self.box.padding[1] - self.box.padding[3]
    self.box:addChild(Text("[font:plain][color:green]"..quest_name, nil, nil, box_size_w))
