...
function ModlandFileSelect:draw()
    ...
    if self.state == "SELECT" or self.state == "TRANSITIONING" then
        setColor(1, 4)
        Draw.printShadow(self:gasterize(Game:locRaw("ui_save_file_copy")), 108, 380)
        setColor(2, 4)
        Draw.printShadow(self:gasterize(Game:locRaw("ui_save_file_erase")), 280, 380)
        if not self.chapter_select then
            setColor(3, 4)
            Draw.printShadow(self:gasterize(Game:locRaw("ui_save_file_chapter_select")), self.bottom_row_heart[3] + 28, 380)
        else
            setColor(3, 4)
            Draw.printShadow(self:gasterize(Game:locRaw("ui_save_file_chapter_select")), self.bottom_row_heart[3] + 28, 380)
            setColor(3, 5)
        ...
        setColor(2, 5)
        Draw.printShadow(self:gasterize(Game:locRaw("ui_save_file_config")), self.bottom_row_heart[2] + 28, 380 + 40)
    else
        setColor(1, 4)
        Draw.printShadow(self:gasterize(Game:locRaw("ui_save_file_cancel")), 110, 380)
    end
